...
def create_librenms(docker_librenms_name, docker_mysql_name, docker_mysql_ip, db_name, db_user_name, db_user_pwd, librenms_network):
    print("建立監控系統總服務")
    print("生成系統認證金鑰碼")
...
def create_mysql(docker_mysql_name, db_root_pwd, db_name, db_user_name, db_user_pwd, librenms_network):
    print("建立監控系統資料庫")
   ...
   # print(mysql_volume)
    docker_mysql_config = 'docker run --net ' + librenms_network + ' --ip "172.3.0.2" --name ' + docker_mysql_name + " -d -e MYSQL_ROOT_PASSWORD=" + db_root_pwd + " -e MYSQL_USER=" + db_user_name + " -e MYSQL_PASSWORD=" + db_user_pwd + " -e MYSQL_DATABASE=" + db_name + " -p 127.0.0.1:3306:3306 -v " + mysql_volume + '/mysql:/var/lib/mysql mysql:5.6 --sql-mode=""'
...
def create_service():
   ...
   # print("librenms_network" + librenms_network)
    librenms_mysql_ip = create_mysql(librenms_mysql_docker_name, librenms_mysql_root_pwd, librenms_mysql_user_db, librenms_mysql_user_name, librenms_mysql_user_pwd, librenms_network)
   # print(librenms_mysql_ip)
    create_librenms(librenms_docker_name, librenms_mysql_docker_name, librenms_mysql_ip, librenms_mysql_user_db, librenms_mysql_user_name, librenms_mysql_user_pwd, librenms_network)
    print("監控系統安裝完成")
