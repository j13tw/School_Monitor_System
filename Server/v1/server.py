    ...
    if request.method == 'POST':
        ...
        if (mysql_connect() == True):
            if (mysql_check_db("school_" + edge_school_id) == False or mysql_check_table("school_" + edge_school_id, "speedtest") == False): 
                return {"uploadSql": "Mysql_DB_Not_Exist_Error"} 
            mysql_conn = MySQLdb.connect(host = mysql_host, \
                port=mysql_port, \
                user=mysql_user, \
                passwd=mysql_passwd, \
            ...
            if (mysql_connection.execute("select * from speedtest where time_logged like '" + str(edge_school_speedtest["timestamp"]).split(" ")[0] + "%%' order by id desc  limit 1") == 0):
...
def edgeNodeSqlUpload():
    if request.method == 'POST':
        ...
        if (mysql_connect() == True):
                ...
                if (mysql_connection.execute("select * from devices where device_id = " + y["device_id"]) == 1):
                    ...
                    try:
                        mysql_connection.execute("UPDATE devices SET \
                            ...
                            os = " + y["os"] + ", status = " + y["status"] + ", status_reason = " + y["status_reason"] + ", disabled = " + y["disabled"] + ", \
                    ...
                    try:
                        mysql_connection.execute("INSERT INTO devices (device_id, hostname, sysName, ip, community, authlevel, authname, authpass, authalgo, cryptopass, cryptoalgo, \
                            ...
                            status, status_reason, disabled, uptime, agent_uptime, last_polled_timetaken, last_discovered_timetaken, last_ping_timetaken, purpose, type, serial, icon, \
                            ...
                            " + y["features"] + ", " + y["location_id"] + ", " + y["os"] + ", " + y["status"] + ", " + y["status_reason"] + ", " + y["disabled"] + ", " + y["uptime"] + ", \
            ...
            # print(edge_device_list)
            cloud_device_list_set = set(cloud_device_list)
