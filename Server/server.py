...
def edgeNodeSqlUpload():
    if request.method == 'POST':
        ...
        for x in range(0, len(edge_school_devices)):
            ...
            if (y["status_reason"] != ""): y["status_reason"] = "'" + str(y["status_reason"]) + "'"
            else: y["status_reason"] = bytes(y["status_reason"], encoding = "utf8").decode('utf-8')
            ...
            if (mysql_connection.execute("select * from devices where device_id = " + y["device_id"]) == 1):
                ...
                status, status_reason, ignores, disabled, uptime, agent_uptime, last_polled, last_poll_attempted, last_polled_timetaken, last_discovered_timetaken, \
